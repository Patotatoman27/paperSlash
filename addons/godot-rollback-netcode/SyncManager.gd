...
const dLogger = preload("res://addons/godot-rollback-netcode/Logger.gd")
...
func start_logging(log_file_path: String, match_info: Dictionary = {}) -> void:
	...
	if not _dLogger:
		_dLogger = dLogger.new(self)
	else:
		_dLogger.stop()

	if _dLogger.start(log_file_path, _network_adaptor.get_unique_id(), match_info) != OK:
		stop_logging()
...
func _update_input_complete_tick() -> void:
	while _input_tick >= _input_complete_tick + 1:
		...
		if _dLogger:
			_dLogger.write_input(input_frame.tick, input_frame.players)
...
func _update_state_hashes() -> void:
	while _state_complete_tick > _last_state_hashed_tick:
		...
		if _dLogger:
			_dLogger.write_state(_last_state_hashed_tick, state_frame.data)
...
func _cleanup_buffers() -> bool:
	...
	while state_buffer.size() > max_buffer_size + 1:
		...
		if input_frame == null:
			...
			if _dLogger:
				_dLogger.data['buffer_underrun_message'] = message
			return false
		if not input_frame.is_complete(_player_peers):
...
func _record_advantage(force_calculate_advantage: bool = false) -> void:
	for peer in _player_peers.values():
		...
		if _dLogger:
			_dLogger.add_value("peer_%s" % peer.peer_id, {
...
func _send_input_messages_to_peer(peer_id: int) -> void:
	...
	if _dLogger:
		_dLogger.data['messages_sent_to_peer_%s' % peer_id] = input_messages.size()
	...
	for input in _get_input_messages_from_send_queue_for_peer(peer):
		...
		if _dLogger:
			_dLogger.add_value("messages_sent_to_peer_%s_size" % peer_id, bytes.size())
			_dLogger.increment_value("messages_sent_to_peer_%s_total_size" % peer_id, bytes.size())
			_dLogger.merge_array_value("input_ticks_sent_to_peer_%s" % peer_id, input.keys())
...
func _physics_process(_delta: float) -> void:
	...
	if _dLogger:
		_dLogger.begin_tick(_current_tick + 1)
		_dLogger.data['input_complete_tick'] = _input_complete_tick
		_dLogger.data['state_complete_tick'] = _state_complete_tick
	...
	if _current_tick == 0:
		...
		if _dLogger:
			...
			_dLogger.write_state(0, state_buffer[0].data)
	...
	if _rollback_ticks > 0:
		if _dLogger:
			_dLogger.data['rollback_ticks'] = _rollback_ticks
			_dLogger.start_timing('rollback')
	...
	if not _mechanized:
		...
		if _ticks_spent_regaining_sync > 0:
			...
			if not _cleanup_buffers():
				...
				if _dLogger:
					_dLogger.skip_tick(dLogger.SkipReason.INPUT_BUFFER_UNDERRUN, start_time)
				return
			...
			if min_lag_to_regain_sync > 0 and _calculate_max_local_lag() > min_lag_to_regain_sync:
				...
				if not _spectating:
					_send_input_messages_to_all_peers()
				if _dLogger:
					_dLogger.skip_tick(dLogger.SkipReason.WAITING_TO_REGAIN_SYNC, start_time)
				return
		...
		if _skip_ticks > 0:
			...
			if _skip_ticks == 0:
				...
				if not _spectating:
					_send_input_messages_to_all_peers()
				if _dLogger:
					_dLogger.skip_tick(dLogger.SkipReason.ADVANTAGE_ADJUSTMENT, start_time)
				return
	...
	if not _mechanized:
		...
		if _spectating:
			...
			if _dLogger:
				_dLogger.data['input_tick'] = _input_tick
	...
	if _current_tick > 0:
		if _dLogger:
			_dLogger.start_timing("current_tick")

		if not _do_tick():
			return

		if _dLogger:
			_dLogger.stop_timing("current_tick")
...
func _process(delta: float) -> void:
	...
	if not interpolation or not _ran_physics_process or _ticks_since_last_interpolation_frame > 1:
		if _dLogger:
			_dLogger.begin_interpolation_frame(_current_tick)
		...
		if _dLogger:
			_dLogger.end_interpolation_frame(start_time)
...
func _on_received_input_tick(peer_id: int, serialized_msg: PackedByteArray) -> void:
	...
	if last_remote_tick > peer.last_remote_input_tick_received:
		...
		for remote_tick in all_remote_ticks:
			...
			if _dLogger:
				_dLogger.add_value('remote_ticks_received_from_%s' % peer_id, remote_tick)
